...
st.caption(
    "FTSE 100 + FTSE 250 operating companies • investment trusts/funds/ETFs excluded • "
    "lowest P/E • latest reported earnings trend • 7-day / 30-day online discussion activity"
)
...
def _extract_constituent_table(html, index_name):
    ...
    for table in tables:
        ...
        # Keep any published industry/sector classification so that
        # investment trusts, funds and similar collective vehicles can
        # be removed from the screening universe.
        classification_col = next(
            (c for c in table.columns
             if any(k in str(c).lower() for k in
                    ["icb sector", "icb subsector", "industry", "sector"])),
            None,
        )
        if classification_col is not None:
            out["Classification"] = table[classification_col].astype(str)
        else:
            out["Classification"] = ""
...
def get_constituents():
    ...
    # Remove collective investment vehicles.  The FTSE 250 in particular
    # contains many investment trusts, whose P/E and EPS are not comparable
    # with normal operating companies.  Property companies/REIT operating
    # businesses are deliberately retained unless they are explicitly
    # classified/named as an investment fund or investment trust.
    company_text = result["Company"].astype(str).str.lower()
    class_text = result.get(
        "Classification", pd.Series("", index=result.index)
    ).astype(str).str.lower()

    class_exclusions = class_text.str.contains(
        r"closed[ -]?end|investment trust|collective investment|"
        r"exchange[ -]?traded|\betf\b|\betc\b|mutual fund|unit trust",
        regex=True, na=False,
    )

    name_exclusions = company_text.str.contains(
        r"investment trust|investment fund|income fund|growth fund|"
        r"opportunities fund|capital trust|venture capital trust|"
        r"\betf\b|\betc\b",
        regex=True, na=False,
    )

    # Many UK investment trusts are simply named '... Trust plc'.
    trust_name = company_text.str.contains(
        r"\btrust(?: plc)?$", regex=True, na=False
    )

    result = result.loc[~(class_exclusions | name_exclusions | trust_name)].copy()
    result["Universe"] = "Operating companies only"
...
Investment trusts, investment funds, ETFs/ETCs and other collective investment vehicles are automatically excluded. Normal operating companies, including listed property/REIT operating companies, remain included.
...
The app starts with the current FTSE 100 and FTSE 250 constituents and automatically removes investment trusts, investment funds, ETFs/ETCs, unit/mutual funds and other collective investment vehicles. Normal listed operating companies remain in the universe.
